...
const HARDCODED_DESKTOP_ICONS = [
  { app: "portfolioApp", name: "My Portfolio", icon: "/sital-photo.jpg" },
  { app: "projectsApp", name: "My Projects", icon: "papirus:apps/preferences-desktop-theme" },
  { app: "kagajAiApp", name: "Kagaj AI", icon: "papirus:apps/gnome-robots" },
  { app: "contactApp", name: "Contact Me", icon: "papirus:apps/internet-mail" },
  ...
  { app: "settingsApp", name: "Settings", icon: "papirus:apps/preferences-system" }
];
...
export class IconManager {
  ...
  async createDesktopFileIcon(fileName, itemData = null) {
    if (fileName.startsWith("About Sital")) return;
  ...
  async loadDesktopItems() {
    ...
    for (const [name, itemData] of Object.entries(desktopFolder)) {
      if (name.startsWith("About Sital") || name === "About Sital.txt" || name === "About Sital.desktop") {
        try {
          await os.fs.unlink(["Desktop", name]);
        } catch {}
        continue;
      }
      if (!itemData.type) {
